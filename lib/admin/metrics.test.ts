...
describe("withinWindow", () => {
  it("is false for undefined", () =>
    expect(withinWindow(undefined, NOW)).toBe(false));
...
describe("computeGameStats", () => {
  ...
  it("does not count abandoned lobbies as active now", () => {
    // Nothing ever closes isOpen on an abandoned lobby, so without the
    // abandonedAt check these would be counted as active forever.
    const games = [
      { isOpen: true, startedAt: NOW - 1000 },
      { isOpen: true, startedAt: NOW - 2000, abandonedAt: NOW - 1000 },
    ];
    expect(computeGameStats(games, NOW).activeNow).toBe(1);
  });
});
...
describe("shouldSetCompletedAt", () => {
  it("true on final round showing results", () =>
    expect(shouldSetCompletedAt("display-results", 3, 3)).toBe(true));
  it("true on final round finishing", () =>
    expect(shouldSetCompletedAt("finished", 3, 3)).toBe(true));
  it("false on display-results of a non-final round", () =>
    expect(shouldSetCompletedAt("display-results", 2, 3)).toBe(false));
  it("false on a non-final round", () =>
    expect(shouldSetCompletedAt("finished", 2, 3)).toBe(false));
  it("false for a non-terminal phase", () =>
    expect(shouldSetCompletedAt("rank-players", 3, 3)).toBe(false));
  it("false for infinite games", () =>
    expect(shouldSetCompletedAt("display-results", 1, 0)).toBe(false));
});

describe("scenarioSortToQuery", () => {
  it("maps popularity and age", () => {
    expect(scenarioSortToQuery("popular-desc")).toEqual({
      index: "byTimesSelected",
      order: "desc",
    });
    expect(scenarioSortToQuery("popular-asc")).toEqual({
      index: "byTimesSelected",
      order: "asc",
    });
    expect(scenarioSortToQuery("newest")).toEqual({
      index: "by_creation_time",
      order: "desc",
    });
    expect(scenarioSortToQuery("oldest")).toEqual({
      index: "by_creation_time",
      order: "asc",
    });
  });
